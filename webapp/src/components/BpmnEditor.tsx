...
import { Alert, App, Badge, Button, Card, Empty, List, Modal, Switch, Tag, Tooltip, Typography } from 'antd'
import { AppstoreOutlined, CloseOutlined, ProfileOutlined, SafetyCertificateOutlined } from '@ant-design/icons'
...
const BpmnEditor = forwardRef<BpmnEditorHandle, Props>(({ xml, forms }, ref) => {
  ...
  const [gridOn, setGridOn] = useState(() => localStorage.getItem(GRID_KEY) !== '0')
  ...
  const applyLintOverlays = useCallback((found: LintIssue[]) => {
    ...
    for (const [elementId, list] of byEl) {
      ...
      try {
        overlays.add(elementId, 'khcn-lint', { position: { top: -6, right: -2 }, html: el })
      } catch {
  ...
  useEffect(() => {
    ...
    modeler
      ...
      .then(() => {
        const canvas = modeler.get('canvas') as { zoom: (m?: string | number) => number }
        ...
        if (localStorage.getItem(GRID_KEY) === '0') {
          ;(modeler.get('grid') as { toggle: (v: boolean) => void }).toggle(false)
        }
        ready = true
        runLiveLint()
      })
  ...
  const undo = () => (modelerRef.current?.get('commandStack') as { undo: () => void } | undefined)?.undo()
  const redo = () => (modelerRef.current?.get('commandStack') as { redo: () => void } | undefined)?.redo()
  const toggleGrid = () => {
    const next = !gridOn
    setGridOn(next)
    localStorage.setItem(GRID_KEY, next ? '1' : '0')
    ;(modelerRef.current?.get('grid') as { toggle: (v: boolean) => void } | undefined)?.toggle(next)
  }
  ...
  const toggleAdvanced = (checked: boolean) => {
    ...
    const modeler = modelerRef.current
    if (!modeler) return
    // Palette rebuild theo sự kiện i18n.changed (API chuẩn của diagram-js).
    ;(modeler.get('eventBus') as { fire: (e: string) => void }).fire('i18n.changed')
    const selection = modeler.get('selection') as { get: () => unknown[]; select: (e: unknown) => void }
  ...
  const focusIssue = (issue: LintIssue) => {
    ...
    const canvas = modeler.get('canvas') as { scrollToElement?: (e: unknown, p?: number) => void }
    ...
    >
      ...
      {importError && (
        <Alert
          ...
          style={{ position: 'absolute', top: 56, left: drawerOpen ? DRAWER_W + 12 : 150, right: panelOpen ? PANEL_W + 12 : 12, zIndex: 28 }}
        />
      )}
      ...
      >
        ...
        <Tooltip title="Kiểm tra tính hợp lệ của quy trình">
          <Badge count={errorCount || warningCount} color={errorCount ? undefined : 'orange'} size="small">
            <Button icon={<SafetyCertificateOutlined />} onClick={runLint}>
              Kiểm tra
            </Button>
          </Badge>
        </Tooltip>
        {isDirty && !importError && <Tag color="warning" style={{ marginInlineEnd: 0 }}>Chưa lưu</Tag>}
      </div>
      ...
      >
        ...
        <Tooltip title={advanced ? 'Đang hiện đầy đủ thuộc tính kỹ thuật' : 'Đang ở chế độ đơn giản cho nghiệp vụ'}>
          <Switch size="small" checked={advanced} onChange={toggleAdvanced} />
        </Tooltip>
      </div>

      {/* Drawer mẫu nghiệp vụ KHCN (dock trái) */}
      <BpmnTemplateDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} onPick={onPickTemplate} />
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ fontSize: 13 }}>
              <Text strong>Bắt đầu nhanh:</Text> ① chọn mẫu trong <Text strong>Mẫu KHCN</Text> (trái) → ② di chuột lên
              canvas, bấm để đặt → ③ nối các bước rồi bấm <Text strong>Kiểm tra</Text>.
            </div>
            <Button size="small" type="text" onClick={dismissHint}>Đã hiểu</Button>
          </div>
        </Card>
      )}
          ...
          <List
            ...
            renderItem={(it) => (
              <List.Item
                ...
                extra={it.elementId ? <Button type="link" size="small">Tới phần tử</Button> : undefined}
              >
