...
- **OCR**: PaddleOCR — called via internal job queue, not inline in request handlers
...
1. Every query that reads or writes SME data must include both `tenant_id` and `client_id` in the `WHERE` clause.
2. Extract `tenant_id` from the authenticated session — never accept it as a user-supplied parameter.
3. Extract `client_id` from validated context — verify the client belongs to the tenant before proceeding.
...
- `when`: timestamp
- `diff`: previous and new values (redact PII where required by NĐ 13/2023)
...
Invoice upload is asynchronous:

1. Route handler receives the file, validates it, stores the raw file to object storage.
2. A job is enqueued (do not call PaddleOCR inline in the handler).
3. The job worker calls PaddleOCR, parses the result, stores structured fields **and** raw OCR output **and** per-field confidence scores.
4. If any field's confidence < 85%, mark the invoice as `needs_review` in the database — do not silently accept.
5. Notify the frontend via WebSocket or polling endpoint that the job is complete.

Never discard raw OCR output. It is needed for debugging and model retraining.

---

## XML / Tax Filing Rules

- HTKK and eTax XML schemas are version-controlled in `config/tax-schemas/`.
- The schema version used for each export must be stored alongside the export record.
- Schema migrations (when GDT releases a new version) go through the architect's review.
- Never hardcode XML namespace strings or version identifiers in domain logic — reference the config.
...
- Accepting `tenant_id` or `client_id` from user-supplied request parameters
...
- **Background job handlers** for async work (OCR, XML generation)
...
- For each API endpoint, test: valid input, invalid input, missing auth, wrong tenant, wrong client
